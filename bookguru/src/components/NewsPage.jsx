...
const SOCKET_URL = API; // same origin where socket server runs
...
export default function NewsPage() {
  ...
  const loadNews = async () => {
    try {
      ...
      // normalized: ensure fields exist (category fallback)
      const normalized = (res.data || []).map(n => ({
        ...n,
        category: n.category || n.category?.trim() || "General",
      }));
      // sort newest first
  ...
  useEffect(() => {
    ...
    // server should emit "newsCreated" with the new news object
    ...
    // optional: server could emit "newsDeleted" or "newsUpdated"
  ...
  useEffect(() => { if (currentPage > totalPages) setCurrentPage(1); }, [totalPages]); // reset if needed
          ...
          >
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }} className="bg-white rounded-2xl max-w-2xl w-full overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              {selectedNews.imageUrl && (
                <img src={`${API}${selectedNews.imageUrl}`} alt={selectedNews.title} className="w-full h-56 object-cover rounded-t-2xl" />
              )}
              <div className="p-6">
